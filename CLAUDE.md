...
| # | Action | Owner |
|---|---|---|
| **P1: Strip base64 previews from autosave payload** | 37% of autosaves silently fail with 400 rejection (28 GB of 75 GB total traffic discarded in 27 days). Root cause: `DATA_UPLOAD_MAX_MEMORY_SIZE = 10 MB` + `editor_state` embedding base64 `data:` previews (~350 kB each, 15–32/order). Fix by removing regenerable previews from persistence; re-render via `fabric-renderer.ts::renderCanvas` on restore. See memory `autosave-payload-bloat-p1.md`. Do NOT raise the limit (causes OOM). Success signal: 400 rate on `PUT` to `canvas-state` reaches zero. | Kanna |
