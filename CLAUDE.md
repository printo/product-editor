...
- **Mobile** — `pinch-utils.ts` + FabricEditor pointer handlers give two-finger pinch→FrameState (Fabric discarded the 2nd touch); CanvasEditorSidebar is a bottom sheet `<md` with safe-area padding; `layout.tsx` exports `viewport.viewportFit:'cover'`. Photo swap is desktop drag-and-drop only (`swapCards` via HTML5 drag) — the tap-to-swap card button was replaced by the per-card Blur Effect toggle (PR #21, Jul 2026), so touch devices currently have no swap gesture.
...
| Photo swap on touch devices | PR #21 (Jul 2026) replaced the tap-to-swap card button with the per-card Blur Effect toggle, so phones/tablets have NO way to swap two photos between cards (desktop drag-and-drop via `swapCards` still works, but touch has no HTML5 drag). If customers ask for it, add a swap affordance that works on touch — e.g. a "Swap with…" action inside the editor modal or a long-press mode. Not blocking; revisit on customer feedback. | Kanna |
