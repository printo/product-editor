...
- **Qty enforcement is deliberately asymmetric**: the ordered quantity (single-surface products only) is a **hard cap going over** and **warn-and-proceed going under**, on *both* sides. Over-upload offers *Keep first N* / *Choose again* in the editor — no proceed-with-all path — and `EditorRenderView` independently rejects an over-count submission with 400, so bypassing the editor doesn't get past it. Under-upload shows the auto-fill / pick-to-fill banner plus a `QtyShortfallWarning` notice in the pre-submit modal, and still submits; **the server accepts a shortfall too**. **Do not make under-upload blocking on either side**: qty is supplied by the caller, so a wrong value would strand a real order at checkout — `qty_violation` has one `>` in it and no lower bound. The rule has exactly two homes, deliberately mirrored: `checkOrderQty` in `src/lib/submit-guards.ts` (pinned by `src/lib/__tests__/submit-guards.test.ts`) and `services/order_qty.py` (pinned by `services/tests/test_order_qty.py`, which re-runs the browser suite's cases). Two further rules on the server half: it is **narrower** than the browser's (it exempts calendars and books outright, where the client's flattened surface count caps them) because being more permissive can never 400 a submission the editor allowed, and it **fails open** — an unreadable layout or an unparseable header means "don't check", never "reject".
- **`qty` lives on `EmbedSession`, not the iframe URL.** It flows caller → `POST /api/embed/session` → session row → embed-proxy cache → `X-Order-Qty` → Django, exactly like `order_id`. The legacy `?qty=N` URL param is a rollout fallback the editor still reads, but **nothing server-side honours it** — it is browser-editable, which is the whole reason the field exists. Anything the proxy injects as a header must also be in its cached session tuple; a field resolved but not cached is present on a session's first request and absent for the remaining 110 minutes.
