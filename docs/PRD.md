...
| ~~**4**~~ | ~~Arm the orphan-export sweep~~ — **shipped 2026-09-10.** `GC_ORPHAN_SWEEP=delete` runs on production; five independent safety guards must all hold before a directory is deleted. Latest health check: 244 dirs scanned, 0 orphans. See `services/orphan_exports.py`. | ~~Kanna~~ | Closed. |
...
| ~~**6**~~ | ~~Sweep expired `EmbedSession` rows~~ — **shipped 2026-09-10 (commit `56a2136`).** `garbage_collector_task` now deletes sessions with `expires_at < (now - 30 days)`, reported under `garbage_collector.embed_sessions_deleted`. | ~~Kanna~~ | Closed. |
...
| ~~**8**~~ | ~~API surface separation~~ — **shipped 2026-09-04 (PR #133, commit `3070706`).** `RenderSubmissionService` now used by both `GenerateLayoutView` and `EditorRenderView`; unified `CanvasData` upsert / `RenderJob` creation / queue selection / dispatch. Plan: [`API_SURFACE_SEPARATION_PRD.md`](API_SURFACE_SEPARATION_PRD.md). | ~~Kanna~~ | Closed. |
| ~~**9**~~ | ~~Clean ~56 ESLint `no-unused-vars` warnings~~ and ~~unify the `LayoutDef` shape (10 `(layoutDef as any)?.surfaces?.[0]` casts in the editor page)~~ — **both shipped.** `LayoutDef` unified 2026-08-14 (commit `713bb5b`) via `getCanvasSpec()`/`getFrames()` helpers. The 56 warnings cleaned up 2026-09-11 (PR #137): fixed the ESLint config's `structuredClone` error, added an `argsIgnorePattern`/`varsIgnorePattern: "^_"` convention, removed dead imports/params/setters, kept a few bindings alive (with disable comments) that back deliberately-hidden UI the codebase asked to preserve. `pnpm lint`/`typecheck`/tests all green. `@typescript-eslint/no-explicit-any` stays `off` — 21 casts remain untouched, tracked separately. | ~~Kanna~~ | Closed. |
