...
SPECTACULAR_SETTINGS = {
    ...
    "DESCRIPTION": (
        ...
        "   https://product-editor.printo.in/editor/layout/<name>?token=<uuid>\n\n"
        "3. Customer edits in canvas, clicks Save & Continue\n\n"
        "4. Product Editor renders the print files on Celery workers, then:\n"
        "   • posts a UX ping to your page (design is being prepared):\n"
        "     window.postMessage({ type: 'pe:render_job', jobId, orderID })\n"
        "   • POSTs a signed webhook to your callback_url with a download_url for\n"
        "     the rendered ZIP (see docs/INTEGRATION.md for the full contract).\n\n"
        "5. If the customer taps the editor's Back button, posts:\n"
        "     window.postMessage({ type: 'pe:back', orderID })\n"
        "   Not browser-history navigation — an iframe shares its tab's back/forward\n"
        "   stack with the parent page, so history.back() from inside it could\n"
        "   navigate your page, or carry the customer off your site entirely. This\n"
        "   message hands you the signal instead; no-op until you add a listener\n"
